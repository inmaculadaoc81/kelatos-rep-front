...
/** Detalle de un lead — modal propio con createPortal(document.body) en
    ...
    NOTA 2026-09-14: el bug de "el clic no hace nada" que llevó a mover
    esto fuera del canvas la primera vez NO era este componente -- era
    /v1/campaigns/:id/leads devolviendo cada lead duplicado (una copia
    "fantasma" del run del Agente LinkedIn encadenado, sin lead_analysis
    propio), lo que producía keys de React repetidas en el .map() de
    abajo y rompía los manejadores de clic tras el primer refresco por
    polling. Arreglado en el backend (filtro por agent_type); este modal
    vuelve al canvas, que es donde el usuario lo quería. */
