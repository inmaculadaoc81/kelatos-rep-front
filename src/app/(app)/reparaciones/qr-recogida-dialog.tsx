...
export function QrRecogidaDialog({
  resguardo,
  open,
  onOpenChange,
}: {
  ...
  useEffect(() => {
    ...
    setError("");
    setCopiado(false);
    fetch(`/api/reparaciones/${resguardo}/qr-recogida`)
      .then((r) => r.json())
      .then((data) => {
      ...
      <DialogContent className="max-w-sm sm:max-w-sm">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <ScanBarcode className="size-5" /> QR de recogida — #{resguardo}
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="flex flex-col items-center gap-3 py-2">
          ...
          {!cargando && !error && dataUrl && (
            ...
            <img src={dataUrl} alt="QR de recogida" className="size-52 rounded-md border" />
          )}
          {!cargando && !error && !dataUrl && (
          ...
          <p className="text-center text-xs text-muted-foreground">
            El cliente escanea este código o abre el enlace para firmar la recogida de su equipo.
          </p>
        </div>
      </DialogContent>
    </Dialog>
