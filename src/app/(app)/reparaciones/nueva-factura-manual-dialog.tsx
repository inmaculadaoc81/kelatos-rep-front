...
// Multiforma ahora vive en el componente compartido selector-forma-pago.tsx
// (petición del usuario, 2026-09-23: extenderla "a todas las facturas y
// tickets" — ampliando la implementación aislada de 2026-09-22 "por el
// momento añádela a factura manual, luego lo pruebo y lo implementamos en
// todos lados").
//
...
export function NuevaFacturaManualDialog({
  ...
  function agregarLineaDesdeStock(p: StockPieza) {
    setLineas((prev) => {
      const nuevas = [...prev, { referencia: p.referencia, descripcion: p.descripcion || p.nombre, cantidad: 1, precio: p.precioCliente }];
      ...
      if (p.manoObra > 0) {
        nuevas.push({ descripcion: `Mano de obra — ${p.descripcion || p.nombre}`, cantidad: 1, precio: p.manoObra });
      }
      return nuevas;
    });
