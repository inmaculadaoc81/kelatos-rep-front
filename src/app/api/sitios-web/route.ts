...
export async function POST(req: Request) {
  ...
  const { nombre, url, slug, tipo } = (await req.json()) as { nombre: string; url?: string; slug?: string; tipo?: string };
  ...
  try {
    const resultado = await kelatosApiPost<{ ok: boolean; sitio: Parameters<typeof mapearSitioWeb>[0] }>(
      "/v1/sitios-web",
      { nombre: nombre.trim(), url: url?.trim() || "", slug: slug?.trim() || "", tipo: tipo?.trim() || "", usuario }
    );
