...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string; companyId: string }> }) {
  ...
  try {
    ...
    const r = await kelatosApiPost<Record<string, unknown>>(
      `/v1/campaigns/${encodeURIComponent(id)}/leads/${encodeURIComponent(companyId)}`,
