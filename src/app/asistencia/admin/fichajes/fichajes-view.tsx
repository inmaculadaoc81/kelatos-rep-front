...
/**
 * Tabla de fichajes reutilizable — la misma vista que usa la
 * Administración general (filtros/agrupar/columnas/dialog de detalle),
 * pero con un modo `soloRemotos` que la restringe a los empleados
 * vinculados a un dispositivo remoto (asistencia.remote_devices). No es
 * un sistema paralelo: mismo endpoint /api/asistencia/admin/fichajes,
 * mismo componente — solo cambia qué empleados puede ver y elegir.
 */
export function FichajesView({ soloRemotos = false }: { soloRemotos?: boolean }) {
  ...
  const listaFiltrada = useMemo(() => aplicarFiltrosColumna(fichajesVisibles), [fichajesVisibles, filtrosColumna]);
  ...
  function opcionesColumna(col: ColumnaFiltrable): string[] {
    const base = aplicarFiltrosColumna(fichajesVisibles, col);
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-lg font-semibold">{soloRemotos ? "Fichajes — Remote Work" : "Fichajes"}</h1>
        {soloRemotos && (
          <p className="text-xs text-muted-foreground">Solo empleados con un dispositivo remoto asignado.</p>
        )}
      </div>
        ...
        <Select value={empleadoId || "__todos"} onValueChange={(v) => setEmpleadoId(!v || v === "__todos" ? "" : v)}>
          <SelectTrigger className="w-48">
            <SelectValue placeholder="Todos los empleados">
              {(v: string) => (!v || v === "__todos" ? "Todos los empleados" : empleadosVisibles.find((e) => String(e.id) === v)?.nombre || "Todos los empleados")}
            </SelectValue>
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="__todos">Todos los empleados</SelectItem>
            {empleadosVisibles.map((e) => <SelectItem key={e.id} value={String(e.id)}>{e.nombre}</SelectItem>)}
          </SelectContent>
        </Select>
