...
/** Solicitudes de corrección de fichaje — misma tabla que Administración,
    con un modo `soloRemotos` para la sección Remote Work. */
export function CorreccionesView({ soloRemotos = false }: { soloRemotos?: boolean }) {
  ...
  const listaFiltrada = useMemo(() => aplicarFiltrosColumna(itemsVisibles), [itemsVisibles, filtrosColumna]);

  function opcionesColumna(col: ColumnaFiltrable): string[] {
    const base = aplicarFiltrosColumna(itemsVisibles, col);
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-lg font-semibold">{soloRemotos ? "Correcciones de fichaje — Remote Work" : "Correcciones de fichaje"}</h1>
        {soloRemotos && <p className="text-xs text-muted-foreground">Solo empleados con un dispositivo remoto asignado.</p>}
      </div>
              ...
              <TableRow key={c.id}>
                <TableCell className="font-medium">
                  <span className="inline-flex items-center gap-2">
                    <Avatar size="sm">
                      <AvatarFallback className={colorAvatar(c.empleado)}>{iniciales(c.empleado)}</AvatarFallback>
                    </Avatar>
                    {c.empleado}
                  </span>
                </TableCell>
