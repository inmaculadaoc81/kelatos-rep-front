...
/** Alta/baja de empleados que pueden fichar — restringido a superadmin,
    ...
    Misma vista para Administración y para Remote Work (soloRemotos) --
    reutilizada, no duplicada, igual que FichajesView y las demás
    pantallas de remote-workers/*: no hay una tabla ni un CRUD paralelo
    de "empleados remotos", es asistencia.empleados filtrado por
    trabaja_remoto (petición del usuario, 2026-09-15: antes "Empleados"
    dentro de Remote Work te sacaba a la vista general sin filtrar). */
export function EmpleadosView({ soloRemotos = false }: { soloRemotos?: boolean }) {
  ...
  const [form, setForm] = useState<FormEmpleado>({ nombre: "", email: "", dni: "", trabajaRemoto: soloRemotos });
  ...
  async function cargar() {
    ...
    try {
      ...
      if (rEmp.ok) {
        const todos = rEmp.empleados as Empleado[];
        setEmpleados(soloRemotos ? todos.filter((e) => e.trabaja_remoto) : todos);
      }
  ...
  function abrirNuevo() {
    setEditando(null);
    setForm({ nombre: "", email: "", dni: "", trabajaRemoto: soloRemotos });
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-semibold">{soloRemotos ? "Empleados remotos" : "Empleados"}</h1>
          <p className="text-xs text-muted-foreground">
            {soloRemotos
              ? "Empleados marcados como \"Trabaja en remoto\" — para ver o cambiar a todos, entra en Empleados desde Asistencia."
              : "Quién puede fichar en el kiosco — dar de alta, editar o quitar acceso."}
          </p>
        </div>
          ...
          <TableBody>
            ...
            {!cargando && empleados.length === 0 && (
              <TableRow><TableCell colSpan={7} className="py-8 text-center text-muted-foreground">
                {soloRemotos ? "Ningún empleado marcado como remoto todavía" : "Sin empleados dados de alta"}
              </TableCell></TableRow>
            )}
